...
*/
//me.skills.push('HTML','JavaScript', 'CSS');
...
/*let booleano = "true";
let stringa = prompt("inserisci il testo")
...
console.log(onlyLetters("I have 4 dogs 12"))*/
...
/*function isThisAnEmail(){
  ...
  if(email.includes("@",".it",".com",".org")){
    console.log("L'email è corretta")
  } else {
    console.log("Questa non è una email")
  }
}
...
/*function isTodayMyBirthday() {
  let oggi = new Date();
  let compleanno = new Date(oggi.getFullYear(), 10 - 1, 1);

  return oggi.getDate() === compleanno.getDate() && oggi.getMonth() === compleanno.getMonth();
}

console.log("Oggi compio gli anni? ", isTodayMyBirthday());
...
/* function newestMovie(){
  let nuovoFilm = [];
  movies.forEach(i => nuovoFilm.push(i.Year))
  nuovoFilm.sort();
  console.log(nuovoFilm[nuovoFilm.length-1]);
}
...
/* function onlyInLastMillennium(){
  let anniNovanta = movies.filter(i => i.Year < 2000)
  return anniNovanta;
}
...
/*function searchByTitle(ricerca) {
  let risultato = movies.filter(film => film.Title.includes(ricerca));
  return risultato;
}
...
/*function searchAndDivide(ricerca){
  ...
  if(movies.forEach(film => film.Title.includes(ricerca))){
    match.push(movies);
  } else {
    unmatch.push(movies)
  }
